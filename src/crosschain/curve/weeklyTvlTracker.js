/**
 * Curve Weekly TVL Tracker
 * Tracks daily TVL stats across Curve Finance for multiple chains
 */
...
async function fetchCurveTVLHistory() {
  try {
    ...
    console.error(chalk.red("Error fetching Curve TVL history:"), error.message);
    return null;
  }
}
...
async function exportToCSV(weeklyData) {
  ...
  if (csvData.length > 0) {
    await writeCSV("output/curve-weekly-tvl.csv", csvData, [
...
async function main() {
  console.log(chalk.cyan("\nFetching Curve weekly TVL data from DefiLlama...\n"));

  const weekDates = getThisWeekDates();
  const protocolData = await fetchCurveTVLHistory();

  if (!protocolData) {
    console.error(chalk.red("Failed to fetch Curve TVL history"));
    process.exit(1);
  }
  ...
  console.log(chalk.gray(`CSV: output/curve-weekly-tvl.csv\n`));
}
