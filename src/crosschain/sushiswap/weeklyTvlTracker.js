/**
 * SushiSwap Weekly TVL Tracker
 * Tracks daily TVL stats across SushiSwap for multiple chains
 */
...
async function fetchSushiSwapTVLHistory() {
  try {
    ...
    console.error(chalk.red("Error fetching SushiSwap TVL history:"), error.message);
    return null;
  }
}
...
function displayWeeklyTVL(weeklyData, weekDates) {
  console.log("\n" + "=".repeat(80));
  console.log(chalk.cyan.bold("  SUSHISWAP - WEEKLY TVL TRACKER"));
...
async function exportToCSV(weeklyData) {
  ...
  if (csvData.length > 0) {
    await writeCSV("output/sushiswap-weekly-tvl.csv", csvData, [
...
async function main() {
  console.log(chalk.cyan("\nFetching SushiSwap weekly TVL data from DefiLlama...\n"));

  const weekDates = getThisWeekDates();
  const protocolData = await fetchSushiSwapTVLHistory();

  if (!protocolData) {
    console.error(chalk.red("Failed to fetch SushiSwap TVL history"));
    process.exit(1);
  }
  ...
  console.log(chalk.gray(`CSV: output/sushiswap-weekly-tvl.csv\n`));
}
