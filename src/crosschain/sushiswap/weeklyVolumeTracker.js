/**
 * SushiSwap Weekly Volume Tracker
 * Tracks daily trading volume stats across SushiSwap for multiple chains
 */
...
const SUSHISWAP_SLUG = "sushiswap";
const CHAIN_MAPPING = {
  ethereum: "Ethereum",
  arbitrum: "Arbitrum",
  optimism: "Optimism",
...
async function fetchSushiSwapVolumeHistory() {
  try {
    const response = await axios.get(
      `${DEFILLAMA_API}/summary/dexs/${SUSHISWAP_SLUG}?excludeTotalDataChart=false&excludeTotalDataChartBreakdown=false`,
      {
    ...
    console.error(chalk.red("Error fetching SushiSwap volume history:"), error.message);
    return null;
  }
}
...
function processWeeklyVolumeData(dexData, weekDates) {
  ...
  for (const entry of dexData.totalDataChartBreakdown) {
    ...
    for (const [chainKey, chainName] of Object.entries(CHAIN_MAPPING)) {
      ...
      let volume = 0;
      if (chainData[chainName] && chainData[chainName]["SushiSwap"]) {
        volume = chainData[chainName]["SushiSwap"];
      }
...
function displayWeeklyVolume(weeklyData, weekDates) {
  console.log("\n" + "=".repeat(80));
  console.log(chalk.cyan.bold("  SUSHISWAP - WEEKLY VOLUME TRACKER"));
...
async function exportToCSV(weeklyData) {
  ...
  if (csvData.length > 0) {
    await writeCSV("output/sushiswap-weekly-volume.csv", csvData, [
...
async function main() {
  console.log(chalk.cyan("\nFetching SushiSwap weekly volume data from DefiLlama...\n"));

  const weekDates = getThisWeekDates();
  const dexData = await fetchSushiSwapVolumeHistory();

  if (!dexData) {
    console.error(chalk.red("Failed to fetch SushiSwap volume history"));
    process.exit(1);
  }
  ...
  console.log(chalk.gray(`CSV: output/sushiswap-weekly-volume.csv\n`));
}
