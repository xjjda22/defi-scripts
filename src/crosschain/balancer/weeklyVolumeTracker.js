/**
 * Balancer Weekly Volume Tracker
 * Tracks daily trading volume stats across Balancer for multiple chains
 */
...
function displayWeeklyVolume(weeklyData, weekDates) {
  console.log("\n" + "=".repeat(80));
  console.log(chalk.cyan.bold("  BALANCER - WEEKLY VOLUME TRACKER"));
...
async function exportToCSV(weeklyData) {
  ...
  if (csvData.length > 0) {
    await writeCSV("output/balancer-weekly-volume.csv", csvData, [
...
async function main() {
  console.log(chalk.cyan("\nFetching Balancer weekly volume data from DefiLlama...\n"));

  const weekDates = getThisWeekDates();
  const dexData = await fetchBalancerVolumeHistory();

  if (!dexData) {
    console.error(chalk.red("Failed to fetch Balancer volume history"));
    process.exit(1);
  }
  ...
  console.log(chalk.gray(`CSV: output/balancer-weekly-volume.csv\n`));
}
