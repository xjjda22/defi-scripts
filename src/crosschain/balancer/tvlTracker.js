/**
 * Balancer TVL Tracker - Cross-Chain Analysis
 * Tracks TVL across Balancer V2/V3 deployments on multiple chains
 */
...
async function fetchBalancerTVL() {
  try {
    ...
    console.error(chalk.red("Error fetching Balancer TVL:"), error.message);
    return null;
  }
}
...
function displayTVL(tvlByChain, totalTVL) {
  console.log("\n" + "=".repeat(80));
  console.log(chalk.cyan.bold("  BALANCER - CROSS-CHAIN TVL TRACKER"));
...
async function main() {
  console.log(chalk.cyan("\nFetching Balancer TVL data from DefiLlama...\n"));

  // Fetch data
  const protocolData = await fetchBalancerTVL();

  if (!protocolData) {
    console.error(chalk.red("Failed to fetch Balancer TVL data"));
    process.exit(1);
  }
  ...
  await writeCSV("balancer_tvl_crosschain.csv", csvData);
  console.log(chalk.green("Data exported to balancer_tvl_crosschain.csv\n"));
}
...
module.exports = { fetchBalancerTVL, processTVLData };
