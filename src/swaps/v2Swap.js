 ...
 */
async function getQuoteForExactOutput(
  chainKey,
  tokenIn,
  tokenOut,
  amountOut,
...
async function swapExactTokensForTokens(
  ...
  const amountOutMin = (
    (BigInt(quote.amountOut) * BigInt(10000 - slippageBps)) /
    BigInt(10000)
  ).toString();
...
async function swapTokensForExactTokens(
  ...
  const quote = await getQuoteForExactOutput(
    chainKey,
    tokenIn,
    tokenOut,
    amountOut,
  ...
  const amountInMax = (
    (BigInt(quote.amountIn) * BigInt(10000 + slippageBps)) /
    BigInt(10000)
  ).toString();
